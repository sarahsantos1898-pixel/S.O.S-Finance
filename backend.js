...
// ============================================
// API DE LOGIN (COM DIFERENCIAÇÃO DE ERRO)
// ============================================
...
app.post('/api/transacao', (req, res) => {
    const { email, tipo, descricao, valor, categoria, cartaoId } = req.body;
    ...
    if (tipo === 'entrada') {
        ...
        // Atualizar gasto do cartão específico
...
app.post('/api/editar-transacao', (req, res) => {
    ...
    // Reverter efeitos da transação original no saldo
    if (tipoOriginal === 'entrada') {
        usuario.saldo -= valorOriginal;
    } else if (tipoOriginal === 'saida') {
        usuario.saldo += valorOriginal;
        
        // Reverter gasto do cartão original
    ...
    if (novoTipo === 'entrada') {
        ...
        // Atualizar gasto do novo cartão
        if (cartaoId && usuario.cartoes) {
...
app.post('/api/excluir-transacao', (req, res) => {
    ...
    // Reverter efeitos no saldo
    if (transacao.tipo === 'entrada') {
        usuario.saldo -= transacao.valor;
    } else if (transacao.tipo === 'saida') {
        usuario.saldo += transacao.valor;
        
        // Reverter gasto do cartão
